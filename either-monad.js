...
// Ex1: Refactor street to use Either instead of nested if's
// =========================
...
street(user); //, {name: "Willow"})
street({}); // "no street")

// Ex1b: Refactor streetName to use Either instead of nested if's
// =========================
...
// we can also let `streetName` decide what to do with the returned value
// USer -> Either Null StreetName
...
// another solution, where we define `parseDbUrl` without fold it
// so we can continue composing

const parseDbUrlFinal = (cfg) =>
  tryCatch(() => JSON.parse(cfg)).map((c) => c.url.match(DB_REGEX));

const startAppFinal = (cfg) =>
  parseDbUrl(cfg)
    .map(([_, user, password, db]) => `starting ${db}, ${user}, ${password}`)
    .fold(
      (nullValue) => 'cant get config',
